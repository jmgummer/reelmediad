...
* This Class Is Used To Return Charts Generated through Fusion Charts Plugin
...
class FusionCharts{
    ...
    public static function packageMentionsXML($client,$narrative, $array,$company_name, $startdate,$enddate,$industry,$backdate){
        ...
        foreach ($array as $key) {
            ...
            $cclient = $key['client_id'];
            $co_value2 = $key['mentions'];
            if($count<10){
                $strXML .= "<set label='".FusionCharts::StripExtra($co_name2)."' value='".$co_value2."' link='P-detailsWin,width=950,height=400,toolbar=no,scrollbars=yes, resizable=no-../drilldown/clientmentions" . urlencode("?client_id=$cclient&startdate=$startdate&enddate=$enddate&industries=$setindustry&mystories=1")."' />";
            }
            
            $count++;
        }
        $companyvalue=IndustryQueries::GetShareVoiceCount($client,$startdate,$enddate,$industry,$backdate);
        $cvalue=$companyvalue['mentions'];
        $strXML .= "<set label='".$company_name."' value='".$cvalue."' link='P-detailsWin,width=950,height=400,toolbar=no,scrollbars=yes, resizable=no-../drilldown/clientmentions" . urlencode("?client_id=$client&startdate=$startdate&enddate=$enddate&industries=$setindustry&mystories=0")."' />";
        $strXML .= "</chart>";
        return $strXML;
    }
