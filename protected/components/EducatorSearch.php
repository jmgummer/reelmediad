		...
		for($x=0; $x<=$my_keyword_count; $x++) 
		{
			if(isset($all_my_keywords[$x])){
				if(trim($all_my_keywords[$x])){
			        $all_my_keywords[$x]=trim($all_my_keywords[$x]);
			        if(strlen($all_my_keywords[$x])>4) {
			            $mention_query.=' Title LIKE "%'. trim(str_replace(' ', '%', $all_my_keywords[$x])) .'%" OR ';	
			        }
			        if(strlen($all_my_keywords[$x])<=4) {
			            $mention_query.=' Title LIKE "%'. trim(str_replace(' ', '%', $all_my_keywords[$x])) .', %" OR ';
			            $mention_query.=' Title LIKE "%'. trim(str_replace(' ', '%', $all_my_keywords[$x])) . '. %" OR ';
		...
		*/
		...
		if($keyquery = Yii::app()->db2->createCommand($sqlkeys)->queryAll()){
			foreach ($keyquery as $keyrow) {
				$keywordid = $keyrow['id'];
				$cleankeywordkey = $keyrow['index_term'];
				$keywordkey = trim(str_replace(' ', '%', $keyrow['index_term']));
				// Get the sub key
				$subquery = "";
				...
				if($subkeyquery = Yii::app()->db2->createCommand($sqlsubkeys)->queryAll()){
					foreach ($subkeyquery as $subkeyrow) {
						$subkeywordkey = trim(str_replace(' ', '%', $subkeyrow['index_term']));
						$subquery .=' Title LIKE "%'.$subkeywordkey.'%" OR ';
					}
				}
				...
				FROM $temp_table WHERE ($subquery Title LIKE '%$keywordkey%')  ORDER BY StoryDate,StoryPage,StoryTime";
				if($query = Yii::app()->db2->createCommand($mentions_sql)->queryAll()){
					$tablerows .= "<tr style='color: #98a7b9;line-height: 21px;' id='$cleankeywordkey'><td style='padding: 8px;line-height: 1.42857143;vertical-align: top;'><font style='color:#505050;font-weight:bold;'>$cleankeywordkey</font></td></tr>";
					foreach ($query as $row) {
						...
						$tablerows .= "<br><font style='color:#505050;'>$Story<br><em><strong>Date: $StoryDate</strong></em><br></font>";
						$tablerows .= "</td>";
						$tablerows .= "</tr>";
					}
				}
