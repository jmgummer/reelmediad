<?php
$this->pageTitle=Yii::app()->name.' | Boolean Search';
$this->breadcrumbs=array('Boolean Search');
?>
...
<li class="active"><a href="#s4" data-toggle="tab">Boolean Search</a></li>
<li class="pull-right">
...
if(isset($_POST['StorySearch'])){
	...
	echo "<em>Please note that this generate yesterday's electronic and today's print stories.The first run is the toughest, it takes an average of 3 minutes. After that its a breeze</em><hr>";
	echo date("H:i:s")."<hr>";
	$initialized = new CustomClassification($clientid,$startdate,$enddate);
	$querykey = $initialized->GetCompanyStories();
	$packaging = new PackageClassified($querykey,$startdate,$enddate,$clientid);
	// echo $packaging;
	$data = $packaging->GetMentions();
	$emaildata = $packaging->PrintEmailManenos();
	echo $emaildata;

	$emaildate = date("F d, Y", strtotime($enddate));

	require 'protected/vendors/phpmailer/PHPMailerAutoload.php';
	$mail = new PHPMailer(); // create a new object
	$mail->IsSMTP(); // enable SMTP
	$mail->IsMail();
	$mail->Host = "62.12.115.139"; // specify main and backup server
	$mail->IsHTML(true);
	$mail->From = "clientservice@reelforge.com";
	$mail->FromName = 'Reelforge Systems';
	$mail->Subject = "Classified Emails | $emaildate";
	$mail->Body = $emaildata;
	// Mail People
	$mail->AddReplyTo('cs@reelforge.com', 'Client Service');
	$mail->AddBCC("steve.oyugi@reelforge.com");
	$mail->AddBCC("prisca.beyo@reelforge.com");
	// Chandaria Users
	$mail->AddBCC("marketing2@chandaria.com");
	$mail->AddBCC("Marketing@chandaria.com");
	$mail->AddBCC("pa2@chandaria.com");
	$mail->AddBCC("Pa@chandaria.com");
	// Send Email
	if(!$mail->Send()) {
